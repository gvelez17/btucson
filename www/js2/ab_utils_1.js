...
function ab_cascade(thislink, catcode) {
	...
	my_left = my_parent_left + 10;

	my_parent_top = Number(parent_elem.offsetTop);
//	if (! my_parent_top) {
//		my_parent_top = 120;
//	}
	my_top = my_parent_top + 10;

alert("parent offsetTop is " + parent_elem.offsetTop);

	mstyle += "z-index: " + mzindex + "; ";  
	mstyle += "left: " + my_left + "px; top: 1.1em; height: auto; width: auto; ";
	mstyle += "position: relative; ";
	mstyle += "background: #ff00ff; ";
	mstyle += "opacity: 1.0; "; 
	mstyle += "border: solid 5px; ";
	mstyle += "padding: 0 0 0 3px; ";

	my_panel.setAttribute("style",mstyle);
	my_panel.menu_part = "submenu";

	// now try setting directly - setting style doesn't work in IE
	my_panel.style.zindex = mzindex;
        my_panel.style.position = "relative";
        my_panel.style.left =  my_left + "px";
        my_panel.style.top = "10px";
